...
class OpenAICompatibleProvider {
      ...
      messages: [
        systemPrompt(),
        {
          ...
          content: [
            ...
            `Return exactly ${request.durationDays} items in {"items":[...]} only.`,
            'Each item must include dayOffset, title, category, objective, keyPoints, caption, shortCta, hashtags, and disclaimerNeeded.',
            'Do not include reelHook or reelScript in this campaign plan; reel scripts are generated by a separate endpoint.',
            `Use zero-based dayOffset values from 0 through ${request.durationDays - 1}; never start at 1.`,
            'Allowed category values are only awareness, myth_buster, symptoms, procedure_explainer, seasonal_health_tip, clinic_service, and faq.',
            'disclaimerNeeded must be a boolean. keyPoints must contain 1 to 5 short strings. hashtags must contain at most 10 short strings.',
            'Keep captions under 450 characters and CTAs under 120 characters.',
            'Use safe patient-education language, no patient-specific examples, no cure guarantees, and no best-clinic claims.'
          ].join('\n')
        }
      ]
    });
  ...
  generateCarouselSlides(request: CarouselGenerationRequest) {
    return this.completeJson({
      ...
      normalize: normalizeCarouselGenerationResponse,
      request,
      messages: [
        systemPrompt(),
        {
          ...
          content: [
            ...
            'Each slide object must include exactly slideNumber, role, headline, body, and visualCue.',
            'Allowed role values are only cover, education, cta, and disclaimer. Use education for symptoms, procedures, myths, FAQs, prevention tips, and service explainers.',
  ...
  private async completeJson<TData>(input: {
    ...
    try {
      ...
      const firstParsed = parseProviderJson(firstAttempt.content, input.schema, input.normalize);
      ...
      const repairedParsed = parseProviderJson(repairAttempt.content, input.schema, input.normalize);
...
function parseProviderJson<TData>(
  ...
  try {
    ...
    return { ok: true, data: schema.parse(normalize ? normalize(parsed) : parsed) };
  } catch {
    return { ok: false };
  }
}

function normalizeCarouselGenerationResponse(value: unknown) {
  ...
  const lastIndex = response.slides.length - 1;
  const slides = response.slides.map((slide, index) => {
    if (!slide || typeof slide !== 'object' || Array.isArray(slide)) {
      return slide;
    }

    const normalizedSlide = { ...(slide as Record<string, unknown>) };
    normalizedSlide.role = normalizeCarouselSlideRole(normalizedSlide.role);
    if (index === 0) {
      normalizedSlide.role = 'cover';
    } else if (index === lastIndex) {
      normalizedSlide.role = 'disclaimer';
    }
  ...
  if (
    slides.length > 2 &&
    !slides.some((slide) => slide && typeof slide === 'object' && !Array.isArray(slide) && slide.role === 'cta')
  ) {
    const ctaIndex = Math.max(1, slides.length - 2);
    const ctaSlide = slides[ctaIndex];
    if (ctaSlide && typeof ctaSlide === 'object' && !Array.isArray(ctaSlide)) {
      slides[ctaIndex] = { ...ctaSlide, role: 'cta' };
    }
  }
...
function normalizeCarouselSlideRole(value: unknown) {
  if (typeof value !== 'string') {
    return 'education';
  }

  const role = value.trim().toLowerCase().replace(/[\s-]+/g, '_');
  if (role === 'cover' || role === 'education' || role === 'cta' || role === 'disclaimer') {
    return role;
  }
  if (role === 'call_to_action' || role === 'action' || role === 'appointment' || role === 'booking') {
    return 'cta';
  }
  if (role === 'legal' || role === 'safety_note' || role === 'closing_disclaimer') {
    return 'disclaimer';
  }
  return 'education';
}
